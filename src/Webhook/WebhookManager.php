	...
	public function reconcile(): void {
		...
		try {
			...
			if ( ! $this->settings->stock_webhook_enabled() ) {
				...
				$this->logger->info( sprintf( 'Webhooks disabled: removed %d subscription(s)', $removed ) );
				return;
			}
			...
			foreach ( $ours as $topic => $subs ) {
				foreach ( $subs as $sub ) {
					...
					if ( $reusable ) {
						...
						$client->delete_webhook( $sub['id'] );
						$this->logger->info( sprintf( 'Webhook removed (stale): %s', $topic ) );
					}
				}
			}
			...
			$this->logger->error( 'Webhook reconcile failed: ' . $exception->status_message() );
		}
	}
