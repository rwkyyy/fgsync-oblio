...
final class OrderListColumn {
	...
	public function add_column( array $columns ): array {
		$columns['oblio_fgwoo'] = __( 'Oblio', 'facturare-gestiune-oblio-woocommerce' );
		return $columns;
	}
	...
	public function render( $column, $order_or_id = null ): void {
		...
		if ( null !== $invoice ) {
			...
			if ( null !== $proforma ) {
				printf(
					...
					esc_html__( 'Proformă', 'facturare-gestiune-oblio-woocommerce' ),
					esc_html( $proforma['series'] ),
					esc_html( $proforma['number'] )
				);
				$has_output = true;
			} elseif ( '' !== (string) $order->get_meta( OrderMeta::key( OrderMeta::TYPE_INVOICE, 'failed' ) ) ) {
				echo '<span style="color:#c62d1c;">' . esc_html__( 'eșec', 'facturare-gestiune-oblio-woocommerce' ) . '</span>';
				$has_output = true;
			}
		}
		...
		if ( null !== OrderMeta::get( $order, OrderMeta::TYPE_STORNO ) ) {
			...
			echo '<span style="display:inline-block;font-size:11px;color:#8a3f0a;background:#fbeedd;padding:1px 6px;border-radius:4px;">' . esc_html__( 'storno', 'facturare-gestiune-oblio-woocommerce' ) . '</span>';
		}
	}
}
