...
final class LogRenderer {
	...
	public static function rows( array $entries ): string {
		if ( empty( $entries ) ) {
			return '<p class="oblio-fgwoo-empty">' . esc_html__( 'Fără intrări.', 'fgsync-oblio' ) . '</p>';
		}
		...
		foreach ( $entries as $entry ) {
			...
			$html .= sprintf(
				'<div class="oblio-fgwoo-logline lvl-%s"><span class="t">%s</span><span class="lvl">%s</span><span class="msg">%s</span></div>',
				esc_attr( $level ),
				esc_html( $entry['time'] ),
